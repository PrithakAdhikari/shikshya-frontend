...
const UpdateCourse = () => {
  ...
  const [thumbnailFile, setThumbnailFile] = useState(null); // ✅ Track new file
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      ...
      if (!publicId) {
        toast.error("No course ID provided in URL.");
        return;
      }

      try {
        const response = await authAxios.get(`${API_URL}api/courses?c=${publicId}`);
        const fetchedCourse = response.data.data;
        const curriculumWithIds = (fetchedCourse.curriculum || []).map((item) => ({
          ...item,
          id: generateId(),
        }));
        ...
        setCourse({
          ...fetchedCourse,
          instructor_user_fk_id: fetchedCourse.instructor_user_fk_id || userId,
          curriculum: curriculumWithIds.length
            ? curriculumWithIds
            : [{ id: generateId(), title: "Section 1", md_text: "" }],
        });
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const courseToSubmit = {
        ...course,
        curriculum: course.curriculum.map(({ id, ...rest }) => rest),
      };

      if (thumbnailFile) {
        // Only send file if it changed
        ...
        response = await authAxios.put(`${API_URL}api/courses?c=${publicId}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      } else {
        delete courseToSubmit.thumbnail;
        delete courseToSubmit.likes;
        delete courseToSubmit.liked;
        response = await authAxios.put(`${API_URL}api/courses?c=${publicId}`, courseToSubmit);
      }

      toast.success("Course successfully updated!");
      console.log("Course updated:", response.data);
      setThumbnailFile(null); // Reset file state after upload
    } catch (error) {
          ...
          {menuOpen && (
            <div className="absolute mt-2 w-64 bg-base-100 rounded-lg shadow z-50 p-4 space-y-4">
              ...
              <div className="p-3 rounded-lg border border-base-300 space-y-3">
                ...
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.files?.[0]) {
                      ...
                      setCourse((prev) => ({ ...prev, thumbnail: URL.createObjectURL(e.target.files[0]) }));
                    }
                  }}
                />
                ...
                <div>
                  <label className="block text-sm font-light mb-1 text-base-content/60">
                    Visibility
                  </label>
              ...
              <div className="flex flex-col items-center space-y-4 pt-4">
                ...
                {!loading ? (
                  ...
                  <button className="btn btn-primary w-full max-w-xs" disabled>
      ...
      <div className="flex h-[calc(100%-6rem)] gap-4">
        ...
        <div className="hidden md:flex flex-col w-1/4 bg-base-100 rounded-lg shadow p-4 overflow-y-auto">
          ...
          <div className="mb-4 p-3 rounded-lg border border-base-300 space-y-3">
            ...
            <div>
              <label className="block text-sm font-light text-base-content/60 mb-1">
                Visibility
              </label>
          ...
          <div className="flex flex-col items-center space-y-4 flex-shrink-0">
            ...
            {!loading ? (
              <button onClick={handleSubmit} className="btn btn-primary max-w-xs">
                Submit
              </button>
            ) : (
              <button className="btn btn-primary max-w-xs" disabled>
                <span className="loading loading-dots loading-sm"></span>
              </button>
            )}
          </div>
